...
use starstream_runtime_next::{Contract, ContractInstance, Host};
use wasmtime::error::Context as _;
use wasmtime::{AsContextMut, Engine, Store, StoreContextMut, bail};

/// Store data used by the Wasm tracing adapter.
///
/// All proving instrumentation state remains owned by
...
pub struct TracedContract<T: 'static> {
    contract: Contract<T>,
    trace_artifacts: Arc<neo_wasm::WasmProgramArtifacts>,
}
...
impl<T: 'static> Clone for TracedContract<T> {
    fn clone(&self) -> Self {
        Self {
            ...
            trace_artifacts: Arc::clone(&self.trace_artifacts),
        }
    }
}
...
impl<T: WasmTraceHost> TracedContract<T> {
    ...
    /// Instantiate the contract and install tracing before calling any export.
    ...
    where
        T: Send,
    {
        let instance = self.contract.instantiate(store.as_context_mut()).await?;
        register_unregistered_core_instance(store.as_context_mut(), &self.trace_artifacts)?;
        Ok(instance)
    }
}
...
fn register_unregistered_core_instance<T: WasmTraceHost>(
    ...
    // Core Wasm start functions execute before `Contract::instantiate` returns.
    // The trace handler reports their instructions through
    // `WasmTraceSink::record_untraced_instance`, since no trace state has
    // been registered yet.
    //
    // TODO: `ContractInstance::instance` exposes the newly created component
    // `Instance`, but Wasmtime currently has no API for obtaining the core
    // debug instances instantiated underneath it. Until it does, scan the
    // entire store and require exactly one core instance without registered
    // trace state. This detects uninstrumented contracts, but the repeated
    // full-store scan still makes n component instantiations cumulatively
    // O(n²). Ideally Wasmtime would expose the component-to-core relationship
    // (for example, `Instance::debug_core_instances`), allowing direct
    // registration here.
