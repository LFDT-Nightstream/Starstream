...
enum ScriptCommand {
    ...
    Call {
        ...
        /// Write the transaction to stdout instead of submitting it.
        #[arg(long)]
        simulate: bool,
    ...
    match command {
        ...
        Command::Contract(ContractCommand::Script(ScriptCommand::Call {
            ...
            let Some(key) = key else {
                info!(%results, "simulated coordination script call");
                let tx = encode_transaction(network, tx)?;
                return stdout()
                    .write_all(&tx)
                    .await
                    .context("failed to write transaction to stdout");
            };
            client.transact(key, tx.clone()).await?;
            let tx = encode_transaction(network, tx)?;
            let digest = Sha256::digest(tx);
            info!(tx = encode_digest(&digest.into()), "submitted transaction");
            stdout()
