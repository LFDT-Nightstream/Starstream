...
Exporting tokens is similar to exporting Utxos, but the exported resource is named `token`,
and there is no context resource.

Token `mint fn`s work like UTXO `main fn`s and have the signature `static func(...) -> token`.

Token `burn fn`s have the signature `static func(this: token) -> ...`.

## Importing tokens

Importing tokens is similar to importing Utxos, but the imported resource is named `token`.
