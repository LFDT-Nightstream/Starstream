...
impl ICPlain {
    ...
    pub fn increment(&mut self, a: &Address<u64>, vt: &MemOp<F>) -> Result<(), SynthesisError> {
    ...
    pub fn increment(
        ...
        let hash_to_field = compress(&array::from_fn(|i| {
            if i == 0 {
                ...
                FpVar::new_witness(cs.clone(), || Ok(F::from(a.tag))).unwrap()
            } else if i == 2 {
                FpVar::new_witness(cs.clone(), || Ok(F::from(vt.timestamp))).unwrap()
            } else {
                vt.values
                    .get(i - 3)
                    .cloned()
