...
/// Wasmtime requires a single pointer's space of TLS. The browser runs this
/// module single-threaded, so a plain atomic global is sufficient.
static WASMTIME_TLS: AtomicUsize = AtomicUsize::new(0);

#[unsafe(no_mangle)]
pub extern "C" fn wasmtime_tls_get() -> *mut u8 {
    WASMTIME_TLS.load(Ordering::Relaxed) as *mut u8
}

#[unsafe(no_mangle)]
pub extern "C" fn wasmtime_tls_set(ptr: *mut u8) {
    WASMTIME_TLS.store(ptr as usize, Ordering::Relaxed);
}
