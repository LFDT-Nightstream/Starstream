...
impl Compiler {
    ...
    fn export_utxo_method(
        ...
        let wit_name = format!("[method]utxo.{}", to_kebab_case(function.name.as_str()));
        if let Some(func_idx) =
            self.make_component_export_wrapper_fn(function.name.span, &sig, core.idx, &core.ty)
        {
