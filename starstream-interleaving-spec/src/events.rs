...
impl EventKind {
    ...
    pub fn decode(
        ...
        for (block_index, (layout_block, block)) in layout.iter().zip(blocks).enumerate() {
            for (word_index, (word, &value)) in layout_block.iter().zip(block).enumerate() {
                if value >= FIELD_MODULUS {
                    return Err(BlockCodecError::NonCanonicalField {
                        block: block_index,
                        word: word_index,
                        value,
                    });
                ...
                let named = |name: &'static str| {
                    u32::try_from(value).map_err(|_| BlockCodecError::NamedArgumentOutOfRange {
                        block: block_index,
                        word: word_index,
                        name,
                        value,
                    ...
                    Word::Constant(expected) => {
                        if value != u64::from(expected) {
                            return if (block_index, word_index) == (0, 0) {
                                Err(BlockCodecError::WrongDiscriminant {
                                    expected: u64::from(expected),
                                    actual: value,
                                })
                            } else {
                                Err(BlockCodecError::UnexpectedConstant {
                                    block: block_index,
                                    word: word_index,
                                    expected: u64::from(expected),
                                    value,
        ...
        if let Some(expected) = expected_method
            ...
            && method != expected
        {
            return Err(BlockCodecError::MethodMismatch {
                expected,
                actual: method,
            });
        }
        ...
        Ok(match self {
            ...
            EventKind::SetStorage => Step::SetStorage {
                ...
                coordinator_handle: coordinator_handle
                    .ok_or(BlockCodecError::MissingCoordinatorHandle)?,
            },
...
mod test {
    use crate::{
        ...
        events::{BlockCodecError, EventKind, encode},
    };
    ...
    fn every_program_event_round_trips_through_its_template() {
        for step in program_steps() {
            // let template = template_for(&step);
    ...
    fn storage_handle_is_host_metadata_not_committed_data() {
        ...
        assert_eq!(
            ...
            Err(BlockCodecError::MissingCoordinatorHandle)
        );
        assert_eq!(
    ...
    fn rejects_named_words_that_do_not_fit_the_wasm_encoding() {
        ...
        assert_eq!(
            kind.decode(&encoded, None, None),
            Err(BlockCodecError::NonCanonicalField {
                block: 0,
                word: 1,
                value: FIELD_MODULUS
            })
    ...
    fn rejects_committed_methods_that_differ_from_the_template() {
        ...
        assert_eq!(
            ...
            Err(BlockCodecError::MethodMismatch {
                expected: other,
                actual: METHOD
            })
        );
