//! Adapter from the ark-based interleaving circuit to neo-fold-clean's R1CS
//! frontend.
//!
//! TODO: also cross-link the Nebula commitment state once the memory API
//! exposes the needed public values.

use crate::F as ArkF;
use crate::circuit::{InterRoundWires, IvcWireLayout, IvcWires, StepCircuitBuilder};
...
pub(crate) fn extract_r1cs_and_assignments(
    ...
    for batch in 0..num_batches {
        ...
        // Fold a batch as one R1CS instance. Intra-batch continuity is enforced
        // here; the folding plan cross-links only the batch boundary.
        let mut first_input: Option<IvcWires> = None;
        let mut prev_output: Option<IvcWires> = None;
...
fn build_plan(
    ...
    // The CE-claim shape depends on the R1CS shape; `fold` retries with the
    // compiler-reported shape if this initial guess is stale.
...
/// Fold the R1CS assignments and return the public shape, plan, and audit.
pub(crate) fn fold(
    shape: &StepShape,
    ...
    // Retry once if the compiler reports the exact CE-claim shape.
...
/// Pull the compiler's CE-claim shape out of a `PostParentShapeMismatch`.
