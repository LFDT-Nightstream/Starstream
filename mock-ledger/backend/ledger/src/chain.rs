...
impl Chain {
    pub async fn new(
        genesis_block: &Vec<WasmComponent>,
        ...
        // load genesis block
        {
            for component_bytes in genesis_block {
                ...
                println!("Loaded contract: {}", contract_hash);

                // Register the utxo
                utxos.insert(
                    UtxoId::new(contract_hash.clone(), 0),
                    UtxoInstance {
                ...
                // Register component with the registry (extracts WIT internally)
                let packages = registry
                    .register(
                        contract_hash,
                        component_bytes,
                        String::from("root"), // TODO: extract actual entrypoint
                    )
                    .await
                    .context("failed to register component WIT")?;

                println!("Registered: {}", packages.main_package_name);
            }
        }
