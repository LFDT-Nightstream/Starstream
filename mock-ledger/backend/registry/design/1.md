...
#### WIT Flattening of a folder (Proof-of-Concept)

The following code demonstrates how to programmatically flatten WIT files coming from a folder with their dependencies into a single output:
...
#### Wit flattening of a Wasm Component (Proof of Concept)
...
match decoded {
    DecodedWasm::Component(resolve, world_id) => {
        // You get a Resolve containing all types, interfaces, worlds
        // and a WorldId pointing to the component's world

        // To print as WIT text:
        let mut printer = WitPrinter::default();
        let pkg_id = resolve.worlds[world_id].package.unwrap();
        printer.print(&resolve, pkg_id, &[])?;
        let wit_text = printer.output.into_string();
    }
    DecodedWasm::WitPackage(resolve, pkg_id) => {
        // Input was an encoded WIT package, not a component
    }
}
```
...
2. **Each UTXO stores WIT types inside them** - possibly minimized version of dependencies.
...
However, there is no good solution do this due to minimization of dependencies.
